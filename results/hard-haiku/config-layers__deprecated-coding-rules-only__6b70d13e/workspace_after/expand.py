...
def expand(value, env):
    if isinstance(value, str):
        # Handle $$ escape by replacing with placeholder
        escaped = value.replace("$$", _ESCAPE_PLACEHOLDER)
        ...
        result = _PATTERN.sub(repl, escaped)
        # Restore escaped $ characters
        return result.replace(_ESCAPE_PLACEHOLDER, "$")
